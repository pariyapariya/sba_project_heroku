...
d3.json('static/data/data_naics.json').then(data => {
    ...
    for(var i = 0; i < values.length; i++){
        values[i].sort((a, b) => b.value - a.value);
    }

    /*
        delete values[i]['BorrName'];
        delete values[i]['GrossApproval'];
        delete values[i]['ApprovalFiscalYear'];
        delete values[i]['NaicsDescription'];
    */
    ...
    for (i = 0; i < len_keys; i++){
        ...
        var obj = {'date': keys[i], 'dataSet': values[i].slice(0,15)}; 
